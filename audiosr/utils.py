...
def lowpass_filtering_prepare_inference(dl_output):
    ...
    filtered_audio = lowpass(waveform.numpy().squeeze(),highcut=cutoff_freq,fs=sampling_rate,order=order,_type=ftype)
    ...
    if(waveform.size(-1) <= filtered_audio.size(-1)):
        ...
        filtered_audio = torch.functional.pad(filtered_audio, (0, waveform.size(-1) - filtered_audio.size(-1)))
...
def mel_spectrogram_train( y):
    ...
    if 24000 not in mel_basis:
        ...
        mel_basis[str(mel_fmax)+'_'+str(y.device)] = torch.from_numpy(mel).float().to(y.device)
        hann_window[str(y.device)] = torch.hann_window(win_length).to(y.device)
    
    y = torch.nn.functional.pad(y.unsqueeze(1), (int((filter_length-hop_length)/2), int((filter_length-hop_length)/2)), mode='reflect')

    y = y.squeeze(1)

    stft_spec = torch.stft(y, filter_length, hop_length=hop_length, win_length=win_length, window=hann_window[str(y.device)],
                    center=False, pad_mode='reflect', normalized=False, onesided=True, return_complex=True)
    
    stft_spec = torch.abs(stft_spec)

    mel = spectral_normalize_torch(torch.matmul(mel_basis[str(mel_fmax)+'_'+str(y.device)], stft_spec))
...
def wav_feature_extraction(waveform, target_frame):
    ...
    log_mel_spec, stft = pad_spec(log_mel_spec, target_frame), pad_spec(stft, target_frame)
...
def read_wav_file(filename):
    ...
    waveform = normalize_wav(waveform) # TODO rescaling the waveform will cause low LSD score

    waveform = waveform[None, ...]
    waveform = pad_wav(
        waveform, target_length=int(48000 * pad_duration)
    )
